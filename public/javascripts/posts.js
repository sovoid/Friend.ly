(function() {
  ...
  function load(loaded) {
      $("#loader").remove();
      if (!loaded) {
          $("#posts").append('<div id="loader" class="col-md-12 text-center"><br><br><img src="/images/logo.gif"></div>');
      }
  }
      ...
      $.ajax(`/api/v1/posts?page=${page}&sort=${sort}`).done(function(posts) {
          ...
          posts.forEach((p) =>
              ...
              <div class="gram-card-header">
                <img src="${p.author.profile_picture}" class="gram-card-user-image lozad">
          ...
          <br>
              <div class="gram-card-image">
                ${
                  p.post.static_url
                    ? `${
                        p.post.type === "png" || p.post.type === "jpg" || p.post.type === "jpeg" || p.post.type === "gif"
                          ? `
                <center>
              ...
              <div class="gram-card-content">
                ...
                <p><a class="gram-card-content-user" href="/u/@${p.author.username}">${p.author.username}</a>
                ${p.post.caption}
                  <span class="label label-info">${p.post.category ? p.post.category : "Unknown"}</span>
                </p>
              ...
              <div class="gram-card-footer">
                <button data="${JSON.stringify(p.post.likes)}" style="text-decoration: none; color: ${
        p.post.likes.find((x) => x == $("#posts").attr("user-id")) ? "grey" : "#f0b917"
      }" class="footer-action-icons likes btn btn-link non-hoverable like-button-box" author="${p.author.id}" id="${p.post._id}-like">
      <i class="glyphicon glyphicon-thumbs-up"></i> ${p.post.likes.length}</button>
          
                <input id="${p.post._id}" class="comments-input comment-input-box" author="${p.author.id}" type="text" id="comment" placeholder="Click enter to comment here..."/>
